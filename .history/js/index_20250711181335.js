...
// نختار زرار Create Account داخل login-2
let createAccButtons = document.querySelectorAll("#login-2 .CreatAcc");

for (let i = 0; i < createAccButtons.length; i++) {
  createAccButtons[i].addEventListener("click", function (e) {
    e.preventDefault(); // لو الزرار <a> يمنع التحميل
    loginTwo.classList.add("d-none"); // نخفي صفحة تسجيل الدخول
    creatAccMealFemal.classList.remove("d-none"); // نظهر صفحة إنشاء الحساب
  });
}
