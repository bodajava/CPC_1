...
let cardsOne = document.querySelectorAll(".custom-card-1");
let lightBoxContainer = document.querySelector("#lightBoxContainer");

// لما تضغط على أي كارد، يظهر الفورم
for (let i = 0; i < cardsOne.length; i++) {
  cardsOne[i].addEventListener("click", function () {
    lightBoxContainer.classList.remove("d-none");
  });
}

// لما تضغط برا الفورم، الفورم يتقفل
lightBoxContainer.addEventListener("click", function (e) {
  // لو ضغطت على الحاوية الخارجية فقط (مش على الفورم نفسه)
  if (e.target === lightBoxContainer) {
    lightBoxContainer.classList.add("d-none");
  }
});

let cardsTwo = document.querySelectorAll(".custom-card-2");
// لما تضغط على أي كارد، يظهر الفورم
for (let i = 0; i < cardsTwo.length; i++) {
  cardsTwo[i].addEventListener("click", function () {
    lightBoxContainer.classList.remove("d-none");
  });
}
