...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const motorOils = document.getElementById("Motor-Oils"); // الـ Section الهدف

  /* يضيف / يزيل الكلاس sticky حسب التقاطع */
  const io = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          navbar.classList.add("sticky"); // أو "fixed-top" إن أردت
        } else {
          navbar.classList.remove("sticky");
        }
      });
    },
    { rootMargin: "-80px 0px 0px 0px", threshold: 0 } // يبدأ قبل 80px تقريبًا
  );
