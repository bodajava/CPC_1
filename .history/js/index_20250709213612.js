...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const motorSection = document.getElementById("Motor-Oils");

  window.addEventListener("scroll", function () {
    const motorTop = motorSection.getBoundingClientRect().top;

    if (motorTop <= 0) {
      navbar.classList.add("navbar-fixed-on-scroll");
    } else {
      navbar.classList.remove("navbar-fixed-on-scroll");
    }
  });
});
