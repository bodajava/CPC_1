...
let acrdTwo = document.querySelectorAll(".custom-card-2");
let acrdThree = document.querySelectorAll(".custom-card-3");

let login = document.querySelector("#login");
let loginTwo = document.querySelector("#login-2");
let loginThree = document.querySelector("#login-3"); // لو فيه login-3 في HTML

// إخفاء كل الـ sections قبل إظهار المطلوب
function hideAllSections() {
  login.classList.add("d-none");
  loginTwo.classList.add("d-none");
  if (loginThree) loginThree.classList.add("d-none");
}
...
for (let i = 0; i < acrdTwo.length; i++) {
  acrdTwo[i].addEventListener("click", function () {
    hideAllSections();
    loginTwo.classList.remove("d-none");
  });
}

for (let i = 0; i < acrdThree.length; i++) {
  acrdThree[i].addEventListener("click", function () {
    hideAllSections();
    if (loginThree) loginThree.classList.remove("d-none");
  });
}
